...
Run some basic validation on the `lines` array. 
        ...
        if ok.A != ok.type lines then throw Error "`lines` isn’t an array"
        if 0 == lines.length then throw Error "`lines` has no elements"
        if ok.S != ok.type lines[0] then throw Error "`lines[0]` isn’t a string"
          ...
          for section in article.sections
            ...
            for job in section.jobs
              switch ok.type job
                ...
                when ok.A # in the form `[ runner, heading, expect, actual ]`
...
A fourth element (of any kind) signifies the fail is just a type-difference. 
          ...
          when '4-html' then """
            <pre class="fail">#{@sanitize @reveal result[0]} (#{ok.type result[0]})</pre>
            ...#{result[1]}...
            <pre class="pass">#{@sanitize @reveal result[2]} (#{ok.type result[2]})</pre>
            """
          when '4-plain' then """
            #{@sanitize @reveal result[0]} (#{ok.type result[0]})
            ...#{result[1]}...
            #{@sanitize @reveal result[2]} (#{ok.type result[2]})
            """
...
#### `is()`
An assertion-runner which expects `ok.type( actual() )` and `expect` to be equal. 
      ...
      is: 
        runner: (expect, actual, mock=[]) ->
          ...
          if error
            [ 'Unexpected exception', error ]
          else if expect != ok.type result
            [ "type #{ok.type result}", 'was returned, but expected', "type #{expect}" ]
...
`test()` method. 
      ...
      match: 
        runner: (expect, actual, mock=[]) ->
          ...
          if error
            [ 'Unexpected exception', error ]
          else if ok.F != typeof expect.test
...
#### `isAssertion()`
Xx. @todo
Note brackets around `ok.O == ok.type line1`, which makes this conditional 
behave as expected! 

      isAssertion: (line1, line2) ->
        return false if ok.F != ok.type line2
        return false if (ok.O == ok.type line1) && ok.F == ok.type line1.runner
        true
...
Create an instance of `Tudor`, to add assertions to. 
    ...
    tudor = new Tudor
      format: if ok.O == typeof window then 'html' else 'plain'
