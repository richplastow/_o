...
var Ok, SomeClass, Tudor, ok, okT, okV, tudor,
...
Tudor = (function() {
  ...
  Tudor.prototype.formatError = function(result) {
    switch (result.length + "-" + this.opt.format) {
      ...
      case '4-html':
        return "<pre class=\"fail\">" + (this.sanitize(this.reveal(result[0]))) + " (" + (ok.type(result[0])) + ")</pre>\n..." + result[1] + "...\n<pre class=\"pass\">" + (this.sanitize(this.reveal(result[2]))) + " (" + (ok.type(result[2])) + ")</pre>";
      case '4-plain':
        return (this.sanitize(this.reveal(result[0]))) + " (" + (ok.type(result[0])) + ")\n..." + result[1] + "...\n" + (this.sanitize(this.reveal(result[2]))) + " (" + (ok.type(result[2])) + ")";
      default:
  ...
  Tudor.prototype.is = {
    runner: function(expect, actual, mock) {
      ...
      if (error) {
        ...
        return ["type " + (ok.type(result)), 'was returned, but expected', "type " + expect];
      }
    }
  };
  ...
  Tudor.prototype.isAssertion = function(line1, line2) {
    if (ok.F !== ok.type(line2)) {
      return false;
    }
    if ((ok.O === ok.type(line1)) && ok.F === ok.type(line1.runner)) {
      return false;
    }
    return true;
  };
...
tudor = new Tudor({
  format: ok.O === typeof window ? 'html' : 'plain'
});

Ok.runTest = tudor["do"];

tudor.add([
  "01 Ok Constructor", tudor.is, "The class and instance are expected types", "The Ok class is a function", ok.F, function() {
    return Ok;
  }, "`new` returns an object", ok.O, function() {
    return new Ok;
  }
]);
}).call(this,this);
